...
BOOL CBetSite_1xbetchina::AnalyzeBetSetRData()
{
	if (m_szTextBet == NULL || m_nTextSizeBet < 20)
		goto L_FALSE;
	//WriteResData(_T("d:\\1xbetchina.rdata.json"), m_szTextBet);
	return TRUE;

L_FALSE:
	WriteResData(_T("d:\\1xbetchina.AnalyzeSetRData.json"), m_szTextBet);
	return FALSE;
}
...
BOOL CBetSite_1xbetchina::AnalyzeBetUpdateCoupon(BetInfo* pBetInfo, int nBetObjNo, float& coeff)
{
	if (m_szTextBet == NULL || m_nTextSizeBet < 20)
		goto L_FALSE;
	WriteResData(_T("d:\\1xbetchina.AnalyzeUpdateCoupon.json"), m_szTextBet);
	{
		Document root;
