...
BOOL SetGameInfoArray1(CArrayGameInfo& aryGameInfo, BetInfo* pBetInfo, int nBetStrategy, float* fBalance)
{
...
DWORD CMainFrame::BetThreadProc()
{
	...
	while (!m_bExitBetThreadFlg)
	{
		...
		g_pBetInfo->Init();
#ifdef NO_ENGINE
		if (SetGameInfoArray1(aryGameInfo, g_pBetInfo, m_nBetStrategy, g_fBalance))
#else
		if (SetGameInfoArray(aryGameInfo, g_pBetInfo, m_nBetStrategy, g_fBalance))
#endif
		{
