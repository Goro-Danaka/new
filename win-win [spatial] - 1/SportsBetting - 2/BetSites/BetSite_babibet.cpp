...
int CBetSite_babibet::MainProc()
{
	...
	if (m_nLogInStatus == LS_SUCCESS)
	{
		...
		else
		{
			...
			{
				OutDbgStrAA("\n[babibet]:MAIN ERROR - REQ_DATA");
				return 0;
			}
			if (!AnalyzeResData())
			{
				OutDbgStrAA("\n[babibet]:MAIN ERROR - AnalyzeResData");
				return 0;
			}
			if (IsNeedGetBalance())
			{
				...
				{
					OutDbgStrAA("\n[babibet]:MAIN ERROR - REQ_BALANCE");
					return 0;
				}
				if (!AnalyzeResBalance())
				{
					OutDbgStrAA("\n[babibet]:MAIN ERROR - AnalyzeResBalance");
					return 0;
				}
			}
			OutDbgStrAA("\n[babibet]****************************");
...
void CBetSite_babibet::RefreshData()
{
	m_nTextSize = 0;
	if (m_bExitThreadFlg)
		return;
...
int CBetSite_babibet::AnalyzeBetREQ_GET_ODD()
{
	if (m_szTextBet == NULL || m_nTextSizeBet < 20)
		goto L_FALSE;
	WriteResData(_T("d:\\babibet.AnalyzeBetREQ_GET_ODD.json"), m_szTextBet);
	{
		Document root;
...
int CBetSite_babibet::AnalyzeBetREQ_UPDATE_BETSLIP(BetInfo* pBetInfo, int nBetObjNo, float& coeff)
{
	...
	if (m_szTextBet == NULL || m_nTextSizeBet < 20)
		goto L_FALSE;
	WriteResData(_T("d:\\babibet.AnalyzeBetREQ_UPDATE_BETSLIP.json"), m_szTextBet);
	{
		Document root;
		ParseResult ok = root.Parse(m_szTextBet);
		if (!ok)
...
int CBetSite_babibet::AnalyzeBetREQ_PLACE_BET()
{
	if (m_szTextBet == NULL || m_nTextSizeBet < 20)
		goto L_FALSE;
	WriteResData(_T("d:\\babibet.AnalyzeBetREQ_PLACE_BET.json"), m_szTextBet);
	{
		Document root;
		ParseResult ok = root.Parse(m_szTextBet);
		if (!ok)
...
BOOL CBetSite_babibet::AnalyzeBetREQ_REMOVE_ODD()
{
	...
	return TRUE;
L_FALSE:
	WriteResData(_T("d:\\babibet.AnalyzeBetREQ_REMOVE_ODD.json"), m_szTextBet);
	return FALSE;
}
...
BOOL CBetSite_babibet::AnalyzeResLogIn()
{
	...
	return TRUE;
L_FALSE:
	WriteResData(_T("d:\\babibet.AnalyzeResLogIn.json"), m_szText);
	return FALSE;
}
...
BOOL CBetSite_babibet::AnalyzeBetResBalance()
{
	...
	return TRUE;
L_FALSE:
	WriteResData(_T("d:\\babibet.AnalyzeBetResBalance.json"), m_szText);
	return FALSE;
}
...
BOOL CBetSite_babibet::AnalyzeResBalance()
{
	...
	return TRUE;
L_FALSE:
	WriteResData(_T("d:\\babibet.AnalyzeResBalance.json"), m_szText);
	return FALSE;
}
...
BOOL CBetSite_babibet::AnalyzeResData()
{
	if (m_szText == NULL || m_nTextSize < 20)
		goto L_FALSE;
	WriteResData(_T("d:\\babibet.AnalyzeResData.json"), m_szText);
	{
		Document root;
		ParseResult ok = root.Parse(m_szText);
		if (!ok)
