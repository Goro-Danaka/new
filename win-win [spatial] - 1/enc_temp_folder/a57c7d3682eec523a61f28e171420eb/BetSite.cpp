...
BOOL CBetSite::AnalyzeResData()
{
	return TRUE;
}

BOOL CBetSite::AnalyzeResBalance()
{
	return TRUE;
}
...
//				pGameInfo->m_bValid = pGameData->m_pGameInfo->m_bValid;
				break;
			}
...
BOOL CBetSite::RefreshData()
{
	...
	{
		OutDbgStrAA("\n[%s]:MAIN ERROR - REQ_DATA", g_szSiteName[m_nBookMaker]);
		return FALSE;
	}
	if (!AnalyzeResData())
	{
		OutDbgStrAA("\n[%s]:MAIN ERROR - AnalyzeResData", g_szSiteName[m_nBookMaker]);
		return FALSE;
	}
	return TRUE;
}

BOOL CBetSite::RefreshBalance()
{
	if (!SendRequestEx(REQ_BALANCE))
	{
		OutDbgStrAA("\n[%s]:MAIN ERROR - REQ_BALANCE", g_szSiteName[m_nBookMaker]);
		return FALSE;
	}
	if (!AnalyzeResBalance())
	{
		OutDbgStrAA("\n[%s]:MAIN ERROR - AnalyzeResBalance", g_szSiteName[m_nBookMaker]);
		return FALSE;
	}
	return TRUE;
}
	...
	while(m_repeat < MAX_REPEAT)
	{
		...
		for (int i = 0; i < 3; i++)
		{
			...
			if (pBetInfo->BetVal[i].bValid)
				nCheckResult[i] = CheckBetInfo(pBetInfo, i, pGameData, &pOddsInfo);
			if (nCheckResult[0] == BET_ODD_REMOVED || nCheckResult[1] == BET_ODD_REMOVED || nCheckResult[2] == BET_ODD_REMOVED ||
				nCheckResult[0] == BET_VALUE_DECREASED || nCheckResult[1] == BET_VALUE_DECREASED || nCheckResult[2] == BET_VALUE_DECREASED)
			{
				pBetInfo->BetVal[nBetObjNo].nResult = nCheckResult[i];
		...
		if (pOddsInfo != NULL)
		{
			float fBalance = g_fBalance[m_nBookMaker];
			...
			{
				if (g_fBalance[m_nBookMaker] < fBalance)
				{
		...
		*ppOddsInfo = pOddsInfo;
		...
		if (pBetInfo->nOddsKind == OI_MAIN)
		{
			if (nBetObjNo == 0)
			{
				...
				if (pBetInfo->BetVal[nBetObjNo].oVal > pOddsInfo->m_oVal1 + 0.001)
			...
			else if (nBetObjNo == 1)
			{
				...
				if (pBetInfo->BetVal[nBetObjNo].oVal > pOddsInfo->m_oVal2 + 0.001)
					return BET_VALUE_DECREASED;
				else
			...
			else if (nBetObjNo == 2)
			{
				...
				if (pBetInfo->BetVal[nBetObjNo].oVal > pOddsInfo->fDraw + 0.001)
					return BET_VALUE_DECREASED;
				else
