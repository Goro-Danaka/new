...
BOOL CBetSite_188bet::AnalyzeBetGetBySelectionId()
{
	if (m_szTextBet == NULL || m_nTextSizeBet < 20)
		goto L_FALSE;
	WriteResData(_T("c:\\188bet.AnalyzeBetGetBySelectionId.json"), m_szTextBet);
	{
		Document root;
...
BOOL CBetSite_188bet::AnalyzeBetGetRemainSelectionId()
{
	...
	return TRUE;
L_FALSE:
	WriteResData(_T("c:\\188bet.AnalyzeBetGetRemainSelectionId.json"), m_szTextBet);
	return FALSE;
}
int nnnn = 0;
int CBetSite_188bet::AnalyzeBetPlaceBetNew()
{
	if (m_szTextBet == NULL || m_nTextSizeBet < 20)
		goto L_FALSE;
	WriteResData(_T("c:\\188bet.AnalyzeBetPlaceBetNew.json"), m_szTextBet);
	{
		Document root;
		ParseResult ok = root.Parse(m_szTextBet);
		if (!ok)
...
BOOL CBetSite_188bet::AnalyzeResLogIn()
{
	...
	return TRUE;
L_FALSE:
	WriteResData(_T("c:\\188bet.AnalyzeResLogIn.json"), m_szText);
	return FALSE;
}
...
BOOL CBetSite_188bet::AnalyzeResLive4()
{
	...
	return TRUE;
L_FALSE:
	WriteResData(_T("c:\\188bet.AnalyzeResLive4.json"), m_szText);
	return FALSE;
}
...
BOOL CBetSite_188bet::AnalyzeResLive2()
{
	if (m_szText == NULL || m_nTextSize < 20)
		goto L_FALSE;
	//WriteResData(_T("c:\\188bet.AnalyzeResLive2.json"), m_szText);
	{
		char* sz = strstr(m_szText, "pd: JSON.parse('");
		if (sz == NULL)
			goto L_FALSE;
...
BOOL CBetSite_188bet::AnalyzeResBalance()
{
	if (m_szText == NULL || m_nTextSize < 20)
		goto L_FALSE;
	WriteResData(_T("c:\\188bet.AnalyzeResBalance.json"), m_szText);
	{
...
BOOL CBetSite_188bet::AnalyzeResData()
{
	if (m_szText == NULL || m_nTextSize < 20)
		goto L_FALSE;
	WriteResData(_T("c:\\188bet.AnalyzeResData.json"), m_szText);
	try
	{
		UINT i, j, k;
		Document root;
