...
int CBetSite_bet888vip::DoBettingEx(BetInfo* pBetInfo, int nBetObjNo, COddsInfo* pOddsInfo)
{
	...
	float coeff = 0;
	if ((nResult = AnalyzeOrderBet(pBetInfo, nBetObjNo, coeff)) != BET_SUCCESS)
	{
...
int CBetSite_bet888vip::AnalyzeOrderSelect()
{
	if (m_szTextBet == NULL || m_nTextSizeBet < 20)
		goto L_FALSE;
	WriteResData(_T("c:\\bet888vip.AnalyzeOrderSelect.json"), m_szTextBet);
	{
		Document root;
...
L_FALSE:
	//WriteResData(_T("c:\\bet888vip.AnalyzeOrderSelect.json"), m_szTextBet);
	return BET_ANALYZE_ERROR1;
}

int CBetSite_bet888vip::AnalyzeOrderBet(BetInfo* pBetInfo, int nBetObjNo, float& coeff)
{
	if (m_szTextBet == NULL || m_nTextSizeBet < 20)
		goto L_FALSE;
	WriteResData(_T("c:\\bet888vip.AnalyzeOrderBet.json"), m_szTextBet);
	{
		Document root;
		ParseResult ok = root.Parse(m_szTextBet);
		if (!ok)
		...
		for (Value::MemberIterator itr2 = data.MemberBegin(); itr2 != data.MemberEnd(); ++itr2)
		{
			...
			{
				if (!GameID.HasMember("tran_odds"))
				{
					coeff = 0;
					goto L_FALSE;
				}
				char* cc;
				cc = (char*)GameID["tran_odds"].GetString();
				sscanf_s(cc, "%f", &coeff);
				if (coeff == 0)
					goto L_FALSE;
				coeff += 0.2f;
			...
			if (bet_code == 600020) // "Sorry,you have exceeded your maximum wger limit 0 !!Please key your wager again!!(600020)",
				return BET_BLOCKED;
			else if (bet_code == 600005 || // Receive disabled, your wager is currently unacceptable.(600005)
...
BOOL CBetSite_bet888vip::AnalyzeBalanceBB()
{
	...
	return TRUE;
L_FALSE:
	WriteResData(_T("c:\\bet888vip.AnalyzeBalanceBB.json"), m_szText);
	return FALSE;
}
...
BOOL CBetSite_bet888vip::AnalyzeBetBalanceBB()
{
	...
	return TRUE;
L_FALSE:
	WriteResData(_T("c:\\bet888vip.AnalyzeBetBalanceBB.json"), m_szText);
	return FALSE;
}
...
BOOL CBetSite_bet888vip::AnalyzeResLive()
{
	...
	return TRUE;
L_FALSE:
	WriteResData(_T("c:\\bet888vip.AnalyzeResLive.json"), m_szText);
	return FALSE;
}
...
BOOL CBetSite_bet888vip::AnalyzeResLive1()
{
	...
	return TRUE;
L_FALSE:
	WriteResData(_T("c:\\bet888vip.AnalyzeResLive1.json"), m_szText);
	return FALSE;
}
...
BOOL CBetSite_bet888vip::AnalyzeResLive2()
{
	...
	return TRUE;
L_FALSE:
	WriteResData(_T("c:\\bet888vip.AnalyzeResLive2.json"), m_szText);
	return FALSE;
}
...
BOOL CBetSite_bet888vip::AnalyzeResLogIn()
{
	...
	return TRUE;
L_FALSE:
	WriteResData(_T("c:\\bet888vip.AnalyzeResLogIn.json"), m_szText);
	return FALSE;
}
...
BOOL CBetSite_bet888vip::AnalyzeResBalance()
{
	...
	return TRUE;
L_FALSE:
	WriteResData(_T("c:\\bet888vip.AnalyzeResBalance.json"), m_szText);
	return FALSE;
}
...
BOOL CBetSite_bet888vip::AnalyzeResData()
{
	if (m_szText == NULL || m_nTextSize < 20)
		goto L_FALSE;
	WriteResData(_T("c:\\bet888vip.AnalyzeResData.json"), m_szText);
	try
	{
