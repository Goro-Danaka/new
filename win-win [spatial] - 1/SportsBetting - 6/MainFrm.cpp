...
CGameInfo* GetBetCandidate(CArrayGameInfo& aryGameInfo, BetInfo* pBetInfo, int nSiteIndex[], int nOddsKind, float fOVal[])
{
	...
	for (int i = 0; i < aryGameInfo.GetSize(); i++)
	{
		...
		if (nObjCount != nObjCount3 && nObjCount != 0)
			continue;
...
BOOL SetGameInfoArray1(CArrayGameInfo& aryGameInfo, BetInfo* pBetInfo, int nBetStrategy, float* fBalance, BOOL bExitBetThreadFlg, BOOL& bBettingCompleted)
{
	bBettingCompleted = FALSE;
	if (bExitBetThreadFlg)
		bBettingCompleted = TRUE;
	int nOddsKind = OI_HANDICAP;// GetRandInfo(OI_TEAM2_GOAL_OU) + 1;//GetRandInfo(OI_TEAM2_GOAL_OU + 1);
	pBetInfo->nCategory = GC_FOOTBALL;// (GetRandInfo(2) == 0) ? GC_FOOTBALL : GC_TENNIS;
	...
	////////////////
	nSiteIndex[0] = SITE_marathonbet;
	nSiteIndex[1] = SITE_188bet;
	nSiteIndex[2] = SITE_18bet;
	nSiteIndex[3] = SITE_1xbetchina;
	nSiteIndex[4] = SITE_babibet;
	nSiteIndex[5] = SITE_bet888vip;
	////////////////
