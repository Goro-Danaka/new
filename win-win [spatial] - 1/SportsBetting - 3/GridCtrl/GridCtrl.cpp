...
BOOL CGridCtrl::DeleteAllText()
{
	UnMergeCells(0, 0, m_nRows - 1, m_nCols - 1);
	for (int row = 0; row < m_nRows; row++)
	{
		for (int col = 0; col < m_nCols; col++)
		{
			//if (row >= m_nFixedRows && col >= m_nFixedCols)
			//	SetItemBkColour(row, col, RGB(0xFF, 0xFF, 0xE0));
...
BOOL CGridCtrl::SetItemInt(int nRow, int nCol, int nVal)
{
	CString str;
...
BOOL CGridCtrl::SetItemFloat(int nRow, int nCol, float fVal)
{
	CString strOld = GetItemText(nRow, nCol);
	TCHAR* szOld = NULL;
	float fValOld;
	if (strOld != "")
	{
		szOld = (LPTSTR)(LPCTSTR)strOld;
		_stscanf_s(szOld, _T("%f"), &fValOld);
		//if ((int)(fValOld * 100) < (int)(fVal) * 100)
		//{
		//	SetItemBkColour(nRow, nCol, RGB(255, 0, 0));
		//}
		//else if ((int)(fValOld * 100) > (int)(fVal * 100))
		//{
		//	SetItemBkColour(nRow, nCol, RGB(0, 255, 0));
		//}
		//else
		//	SetItemBkColour(nRow, nCol);
	}
	else
		SetItemFgColour(nRow, nCol);

	CString str;
	str.Format(_T("%.2f"), fVal);
	SetItemText(nRow, nCol, str);
	return TRUE;
}
