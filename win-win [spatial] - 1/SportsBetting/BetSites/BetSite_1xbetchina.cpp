...
int CBetSite_1xbetchina::AnalyzeBetPutBet(int& waitTime)
{
	...
	WriteResData(_T("c:\\1xbetchina.AnalyzePutBet.json"), m_szTextBet);
	{
		...
		if (nErr != 0)
		{
			...
			if (nErr == 135)
				return BET_BLOCKED;
			if (nErr == 140) // "One of the selected games is absent from Live"
				return BET_BLOCKED;
...
int CBetSite_1xbetchina::AnalyzeBetPutBet1()
{
	if (m_szTextBet == NULL || m_nTextSizeBet < 20)
		goto L_FALSE;
	WriteResData(_T("c:\\1xbetchina.AnalyzePutBet1.json"), m_szTextBet);
	{
		Document root;
		ParseResult ok = root.Parse(m_szTextBet);
		if (!ok)
...
BOOL CBetSite_1xbetchina::AnalyzeResRedirect()
{
	...
	return TRUE;
L_FALSE:
	WriteResData(_T("c:\\1xbetchina.AnalyzeResRedirect.json"), m_szText);
	return FALSE;
}
...
BOOL CBetSite_1xbetchina::AnalyzeResLogIn()
{
	...
	//WriteResData(_T("c:\\1xbetchina.login.json"), m_szText);
	{
		Document root;
	...
	return TRUE;
L_FALSE:
	WriteResData(_T("c:\\1xbetchina.AnalyzeResLogIn.json"), m_szText);
	return FALSE;
}
...
BOOL CBetSite_1xbetchina::AnalyzeResBalance()
{
	if (m_szText == NULL || m_nTextSize < 20)
		goto L_FALSE;
	//WriteResData(_T("c:\\1xbetchina.AnalyzeResBalance.json"), m_szText);
	{
		Document root;
		ParseResult ok = root.Parse(m_szText);
		if (!ok)
...
BOOL CBetSite_1xbetchina::AnalyzeBetResBalance(int nStep, float& fMoney)
{
	if (m_szTextBet == NULL || m_nTextSizeBet < 20)
		goto L_FALSE;
	//WriteResData(_T("c:\\1xbetchina.AnalyzeBetResBalance.json"), m_szTextBet);
	{
		Document root;
		ParseResult ok = root.Parse(m_szTextBet);
		if (!ok)
...
BOOL CBetSite_1xbetchina::AnalyzeResData()
{
	if (m_szText == NULL || m_nTextSize < 20)
		goto L_FALSE;
	WriteResData(_T("c:\\1xbetchina.AnalyzeResData.json"), m_szText);
	try
	{
		UINT i, j;
