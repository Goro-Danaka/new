...
BOOL CBetSite_bwin::AnalyzeResData()
{
	if (m_szText == NULL || strlen(m_szText) < 20)
		goto L_FALSE;
	WriteResData(_T("c:\\bwin.AnalyzeResData.json"), m_szText);
	{
		Document root;
...
BOOL CBetSite_bwin::AnalyzePlaceBetNew()
{
	...
	return TRUE;
L_FALSE:
	WriteResData(_T("c:\\bwin.AnalyzePlaceBetNew.json"), m_szTextBet);
	return FALSE;
}
...
BOOL CBetSite_bwin::AnalyzeBetSlipSave()
{
	...
	return TRUE;
L_FALSE:
	WriteResData(_T("c:\\bwin.AnalyzeBetSlipSave.json"), m_szTextBet);
	return FALSE;
}
...
BOOL CBetSite_bwin::AnalyzeResData1(char* szText)
{
		...
		goto L_FALSE;
	{
		...
		_stprintf_s(szFile, _T("c:\\bwin.com.thread_%d.json"), nID);
	...
	return TRUE;
L_FALSE:
	WriteResData(_T("c:\\bwin.AnalyzeResData1.json"), m_szTextBet);
	return FALSE;
}
#endif
...
BOOL CBetSite_bwin::AnalyzeResBalance()
{
	...
	return TRUE;
L_FALSE:
	WriteResData(_T("c:\\bwin.AnalyzeResBalance.json"), m_szTextBet);
	return FALSE;
}
...
BOOL CBetSite_bwin::AnalyzeResLive()
{
	...
	return TRUE;
L_FALSE:
	WriteResData(_T("c:\\bwin.AnalyzeResLive.json"), m_szTextBet);
	return FALSE;
}
