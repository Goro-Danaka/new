...
//https://www.18bet.com/en/sport/live

CBetSite_18bet::CBetSite_18bet()
{
	m_nBookMaker = SITE_18bet;
	m_nMinBetMoney = 8;
...
int CBetSite_18bet::MainProc()
{
	...
	if (m_nLogInStatus == LS_SUCCESS)
	{
		...
		else
		{
			...
			//{
			//	OutDbgStrAA("\n[18bet]:MAIN ERROR - REQ_HB");
			//	return 0;
			//}
			if (!SendRequestEx(REQ_DATA))
			{
				OutDbgStrAA("\n[18bet]:MAIN ERROR - REQ_DATA");
				return 0;
			}
			if (!AnalyzeResData())
			{
				OutDbgStrAA("\n[18bet]:MAIN ERROR - AnalyzeResData");
				return 0;
			}
			if (IsNeedGetBalance())
			{
				...
				{
					OutDbgStrAA("\n[18bet]:MAIN ERROR - REQ_BALANCE");
					return 0;
				}
				if (!AnalyzeResBalance())
				{
					OutDbgStrAA("\n[18bet]:MAIN ERROR - AnalyzeResBalance");
					return 0;
				}
			}
			OutDbgStrAA("\n[18bet]****************************");
	...
	else
	{
		...
		else
		{
			if ((m_nLogInStatus == LS_INIT || m_nLogInStatus == LS_LIVE) && strcmp(m_szID, "") != 0 && strcmp(m_szPswd, "") != 0)
			{
				...
				{
					OutDbgStrAA("\n[18bet]:MAIN ERROR - REQ_LOGIN");
					return 0;
				}
				if (!AnalyzeResLogIn())
				{
					OutDbgStrAA("\n[18bet]:MAIN ERROR - AnalyzeResLogIn");
					return 0;
				}
				if (m_nLogInStatus == LS_SUCCESS)
				{
					...
					{
						OutDbgStrAA("\n[18bet]:MAIN ERROR - REQ_LIVE3");
						return 0;
					}
					if (!SendRequestEx(REQ_LIVE2))
					{
						OutDbgStrAA("\n[18bet]:MAIN ERROR - REQ_LIVE2");
						return 0;
					}
					if (!SendRequestEx(REQ_LIVE5))
					{
						OutDbgStrAA("\n[18bet]:MAIN ERROR - REQ_LIVE5");
						return 0;
					}
					if (!SendRequestEx(REQ_LIVE1))
					{
						OutDbgStrAA("\n[18bet]:MAIN ERROR - REQ_LIVE1");
						return 0;
					}
					if (!SendRequestEx(REQ_LIVE4))
					{
						OutDbgStrAA("\n[18bet]:MAIN ERROR - REQ_LIVE4");
						return 0;
					}
				}
			}
...
void CBetSite_18bet::RefreshData()
{
	m_nTextSize = 0;
	if (m_bExitThreadFlg)
		return;
...
BOOL CBetSite_18bet::SendRequest(int nReqKind)
{
	CHttpConnection* pServer = NULL;
	m_nTextSize = 0;

	DWORD dwRet;
	CString strServerName = _T("www.18bet.com");
	...
	try
	{
		...
		else if (nReqKind == REQ_LOGIN)
		{
			...
			m_pSession->SetCookie(_T("https://www.18bet.com"), _T("DELASESS"), m_szSession);

			//strcpy_s(strFormData, "sport_id=1&time_range=today&method=getEuroSport");
			sprintf_s(strFormData, "login_form[username]=%s&login_form[password]=%s&method=login&system_data[current_url]=https://www.18bet.com/en/sport/live", m_szID, m_szPswd);
		}
		else if (nReqKind == REQ_LIVE3)
		{
			...
			m_HttpFile->AddRequestHeaders(_T("Accept-Encoding:gzip, deflate, br"));
			m_HttpFile->AddRequestHeaders(_T("referer:https://www.18bet.com/en/sport/live"));
			m_pSession->SetCookie(_T("https://www.18bet.com"), _T("DELASESS"), m_szSession);
		}
		else if (nReqKind == REQ_LIVE2)
		{
			...
			m_HttpFile->AddRequestHeaders(_T("Accept-Encoding:gzip, deflate, br"));
			m_HttpFile->AddRequestHeaders(_T("Referer:https://www.18bet.com/zh/euro/live"));
			m_HttpFile->AddRequestHeaders(_T("x-requested-with:XMLHttpRequest"));
			m_pSession->SetCookie(_T("https://www.18bet.com"), _T("DELASESS"), m_szSession);

			strcpy_s(strFormData, "method=serviceOneTwo&system_data[current_url]=https://www.18bet.com/zh/euro/live");
		}
		...
		else if (nReqKind == REQ_LIVE4)
		{
			...
			m_HttpFile->AddRequestHeaders(_T("Accept:text/html, application/xhtml+xml, */*"));
			m_HttpFile->AddRequestHeaders(_T("Accept-Encoding:gzip, deflate"));
			m_HttpFile->AddRequestHeaders(_T("Referer:https://www.18bet.com/en/euro/live"));
			m_pSession->SetCookie(_T("https://www.18bet.com"), _T("DELASESS"), m_szSession);
		}
...
UINT64 CBetSite_18bet::GetOddID(int nBetObjNo, COddsInfo* pOddsInfo)
{
	if (nBetObjNo == 0)
...
int CBetSite_18bet::SendBetRequest(BetInfo* pBetInfo, int nBetObjNo, int nReqKind)
{
	m_nTextSizeBet = 0;
	CHttpConnection* pServer = NULL;
	DWORD dwRet;
	...
	try
	{
		...
		if (nReqKind == REQ_GET_ODD)
		{
			...
			m_HttpFileBet->AddRequestHeaders(_T("referer:https://www.18bet.com/en/sport/live"));
			m_pSessionBet->SetCookie(_T("https://www.18bet.com"), _T("DELASESS"), m_szSession);
		...
		else if (nReqKind == REQ_UPDATE_BETSLIP)
		{
			...
			m_HttpFileBet->AddRequestHeaders(_T("referer:https://www.18bet.com/en/sport/live"));
			m_pSessionBet->SetCookie(_T("https://www.18bet.com"), _T("DELASESS"), m_szSession);
			//m_pSessionBet->SetCookie(_T("https://www.18bet.com"), _T("sv"), m_szSV);
		...
		else if (nReqKind == REQ_PLACE_BET)
		{
		...
		else if (nReqKind == REQ_REMOVE_ODD)
		{
		...
		else if (nReqKind == REQ_BET_BALANCE)
		{
...
int CBetSite_18bet::DoBettingEx(BetInfo* pBetInfo, int nBetObjNo, COddsInfo* pOddsInfo)
{
	m_dwOddID = GetOddID(nBetObjNo, pOddsInfo);

	int nResult;
	if ((nResult = SendBetRequestEx(pBetInfo, nBetObjNo, REQ_GET_ODD)) != BET_SUCCESS)
	{
		OutDbgStrAA("\n[18bet]:BET ERROR - BET_GET_ODD_FAIL (%.2f)", pBetInfo->hVal1);
		pBetInfo->BetVal[nBetObjNo].nResult = BET_GET_ODD_FAIL;
		goto L_END;
	}
	if ((nResult = AnalyzeBetREQ_GET_ODD()) != BET_SUCCESS)
	{
		OutDbgStrAA("\n[18bet]:BET ERROR - AnalyzeBetREQ_GET_ODD");
		pBetInfo->BetVal[nBetObjNo].nResult = nResult;
		goto L_END;
	}
	if ((nResult = SendBetRequestEx(pBetInfo, nBetObjNo, REQ_UPDATE_BETSLIP)) != BET_SUCCESS)
	{
		OutDbgStrAA("\n[18bet]:BET ERROR - REQ_UPDATE_BETSLIP");
	...
	{
		OutDbgStrAA("\n[18bet]:BET ERROR - AnalyzeBetREQ_UPDATE_BETSLIP");
		pBetInfo->BetVal[nBetObjNo].nResult = nResult;
		goto L_END;
	}
	if (coeff == 0)
	...
	{
		OutDbgStrAA("\n[18bet]:BET ERROR - Coeff Decreased");
		pBetInfo->BetVal[nBetObjNo].nResult = BET_VALUE_DECREASED;
		goto L_END;
	}
	if ((nResult = SendBetRequestEx(pBetInfo, nBetObjNo, REQ_PLACE_BET)) != BET_SUCCESS)
	{
		OutDbgStrAA("\n[18bet]:BET ERROR - REQ_PLACE_BET");
		pBetInfo->BetVal[nBetObjNo].nResult = REQ_PLACE_BET_FAIL;
		goto L_END;
	}
	if ((nResult = AnalyzeBetREQ_PLACE_BET()) != BET_SUCCESS)
	{
		OutDbgStrAA("\n[18bet]:BET ERROR - AnalyzeBetREQ_PLACE_BET");
		pBetInfo->BetVal[nBetObjNo].nResult = nResult;
		goto L_END;
	}
	if ((nResult = SendBetRequestEx(pBetInfo, nBetObjNo, REQ_REMOVE_ODD)) != BET_SUCCESS)
	{
		OutDbgStrAA("\n[18bet]:BET ERROR - REQ_REMOVE_ODD_FAIL");
		pBetInfo->BetVal[nBetObjNo].nResult = BET_SUCCESS;// nResult;
		goto L_END;
	}
	if (!AnalyzeBetREQ_REMOVE_ODD())
	{
		OutDbgStrAA("\n[18bet]:BET ERROR - AnalyzeBetREQ_REMOVE_ODD");
		pBetInfo->BetVal[nBetObjNo].nResult = BET_SUCCESS;// BET_ANALYZE_ERROR4;
		goto L_END;
	}
	if ((nResult = SendBetRequestEx(pBetInfo, nBetObjNo, REQ_BET_BALANCE)) != BET_SUCCESS)
	{
		OutDbgStrAA("\n[18bet]:BET ERROR - REQ_BET_BALANCE");
		pBetInfo->BetVal[nBetObjNo].nResult = BET_SUCCESS;
		return 0;
	}
	if (!AnalyzeBetResBalance())
	{
		OutDbgStrAA("\n[18bet]:BET ERROR - AnalyzeBetResBalance");
		pBetInfo->BetVal[nBetObjNo].nResult = BET_SUCCESS;
		return 0;
	}
...
int CBetSite_18bet::AnalyzeBetREQ_GET_ODD()
{
	if (m_szTextBet == NULL || m_nTextSizeBet < 20)
		goto L_FALSE;
	WriteResData(_T("d:\\18bet.AnalyzeBetREQ_GET_ODD.json"), m_szTextBet);
	{
		Document root;
...
int CBetSite_18bet::AnalyzeBetREQ_UPDATE_BETSLIP(BetInfo* pBetInfo, int nBetObjNo, float& coeff)
{
	coeff = 0;
	if (m_szTextBet == NULL || m_nTextSizeBet < 20)
		goto L_FALSE;
	WriteResData(_T("d:\\18bet.AnalyzeBetREQ_UPDATE_BETSLIP.json"), m_szTextBet);
	{
		Document root;
		ParseResult ok = root.Parse(m_szTextBet);
		if (!ok)
...
int CBetSite_18bet::AnalyzeBetREQ_PLACE_BET()
{
	if (m_szTextBet == NULL || m_nTextSizeBet < 20)
		goto L_FALSE;
	WriteResData(_T("d:\\18bet.AnalyzeBetREQ_PLACE_BET.json"), m_szTextBet);
	{
		Document root;
		ParseResult ok = root.Parse(m_szTextBet);
		if (!ok)
...
BOOL CBetSite_18bet::AnalyzeBetREQ_REMOVE_ODD()
{
	if (m_szTextBet == NULL || m_nTextSizeBet < 20)
		goto L_FALSE;
	{
	...
	return TRUE;
L_FALSE:
	WriteResData(_T("d:\\18bet.AnalyzeBetREQ_REMOVE_ODD.json"), m_szTextBet);
	return FALSE;
}

BOOL CBetSite_18bet::AnalyzeResLogIn()
{
	...
	return TRUE;
L_FALSE:
	WriteResData(_T("d:\\18bet.AnalyzeResLogIn.json"), m_szText);
	return FALSE;
}

BOOL CBetSite_18bet::AnalyzeResLogOut()
{
...
BOOL CBetSite_18bet::AnalyzeBetResBalance()
{
	if (m_szTextBet == NULL || m_nTextSizeBet < 20)
		goto L_FALSE;
	{
	...
	return TRUE;
L_FALSE:
	WriteResData(_T("d:\\18bet.AnalyzeBetResBalance.json"), m_szText);
	return FALSE;
}

BOOL CBetSite_18bet::AnalyzeResBalance()
{
	if (m_szText == NULL || m_nTextSize < 20)
		goto L_FALSE;
	{
	...
	return TRUE;
L_FALSE:
	WriteResData(_T("d:\\18bet.AnalyzeResBalance.json"), m_szText);
	return FALSE;
}

BOOL CBetSite_18bet::AnalyzeResData()
{
	if (m_szText == NULL || m_nTextSize < 20)
		goto L_FALSE;
	WriteResData(_T("d:\\18bet.AnalyzeResData.json"), m_szText);
	{
		Document root;
		ParseResult ok = root.Parse(m_szText);
		if (!ok)
...
int CBetSite_18bet::GetGameCategory(char* szGameCategory)
{
...
CString CBetSite_18bet::GetGameCategory(int nGameCategory)
{
...
BOOL CBetSite_18bet::UpdateTree(int nIsExitThread)
{
...
COddsInfo* CBetSite_18bet::MyAddOddInfo(CGameData* pGameData, int nOddsKind, float hVal1, float fOval1, float fOval2, float fOval3, UINT64 dwOddID1, UINT64 dwOddID2, UINT64 dwOddID3)
{
...
void CBetSite_18bet::GetOddVals(int nGameCategory, CGameData* pGameData, Value& info, char* szItem)
{
	char* num = NULL;
	char szItem1[0x20];
	char szItem2[0x20];
...
void CBetSite_18bet::GetBalance(char* szAmount)
{//"1,034.14 CNY"
...
BOOL CBetSite_18bet::GetDELASESS(TCHAR* buffer)
{
...
BOOL CBetSite_18bet::GetSV(TCHAR* buffer)
{
	TCHAR szSV[0x100];
