...
int CBetSite_bet888vip::MainProc()
{
	...
	if (m_nLogInStatus == LS_SUCCESS)
	{
		...
		else
		{
			...
			{
				OutDbgStrAA("\n[bet888vip]:MAIN ERROR - REQ_DATA");
				return 0;
			}
			if (!AnalyzeResData())
			{
				OutDbgStrAA("\n[bet888vip]:MAIN ERROR - AnalyzeResData");
				return 0;
			}
			if (IsNeedGetBalance())
			{
				if (!SendRequestEx(REQ_BALANCE_BB))
				{
					OutDbgStrAA("\n[bet888vip]:MAIN ERROR - REQ_BALANCE_BB");
					return 0;
				}
				if (!AnalyzeBalanceBB())
				{
					OutDbgStrAA("\n[bet888vip]:MAIN ERROR - AnalyzeBalanceBB");
					return 0;
				}
			}
			OutDbgStrAA("\n[bet888vip]****************************");
...
void CBetSite_bet888vip::RefreshData()
{
	m_nTextSize = 0;
	if (m_bExitThreadFlg)
		return;
