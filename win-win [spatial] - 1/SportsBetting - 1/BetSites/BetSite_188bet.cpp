...
int CBetSite_188bet::MainProc()
{
	...
	if (m_nLogInStatus == LS_SUCCESS)
	{
		...
		else
		{
			...
			{
				OutDbgStrAA("\n[188bet]:MAIN ERROR - REQ_DATA");
				return 0;
			}
			if (!AnalyzeResData())
			{
				OutDbgStrAA("\n[188bet]:MAIN ERROR - AnalyzeResData");
				return 0;
			}
			if (IsNeedGetBalance())
			{
				...
				{
					OutDbgStrAA("\n[188bet]:MAIN ERROR - REQ_BALANCE");
					return 0;
				}
				if (!AnalyzeResBalance())
				{
					OutDbgStrAA("\n[188bet]:MAIN ERROR - AnalyzeResBalance");
					return 0;
				}
			}
			OutDbgStrAA("\n[188bet]****************************");
...
void CBetSite_188bet::RefreshData()
{
	m_nTextSize = 0;
	if (m_bExitThreadFlg)
		return;
