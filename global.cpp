...
BOOL SplitS(char* str, char* c, string& s1, string& s2)
{
	...
	int nColonCount = 0, i = 0, nColPos = 0;
	char* temp = (char*)str;
	char* szColonPos;

	szColonPos = strstr(temp, c);
	if (szColonPos == NULL)
		return FALSE;
	nColPos = szColonPos - temp;

	char ss1[0x40], ss2[0x40];
	memcpy(ss1, str, nColPos);
	ss1[nColPos] = 0;
	memcpy(ss2, str + nColPos + strlen(c), strlen(str) - nColPos - strlen(c));
	ss2[strlen(str) - nColPos - strlen(c)] = 0;
	::rtrim(ss2, '\n');
